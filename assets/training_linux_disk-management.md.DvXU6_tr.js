import{_ as a,c as n,o as p,a6 as e}from"./chunks/framework.DwkvEpDD.js";const u=JSON.parse('{"title":"Linux 磁盘管理 - 记一次分区格式化","description":"通过一次虚拟化平台服务器新加磁盘的操作，讨论 Linux 平台磁盘管理、分区格式化的操作。","frontmatter":{"title":"Linux 磁盘管理 - 记一次分区格式化","description":"通过一次虚拟化平台服务器新加磁盘的操作，讨论 Linux 平台磁盘管理、分区格式化的操作。","categories":["折腾日记"],"tags":["Linux","运维","磁盘管理"],"outline":[2,3],"date":"2025-04-25T17:20:00.000Z","head":[["meta",{"name":"keywords","content":"Linux, 运维, fdisk, parted, 磁盘管理, 磁盘工具, 分区格式化"}]]},"headers":[],"relativePath":"training/linux/disk-management.md","filePath":"training/linux/disk-management.md","lastUpdated":1745572258000}'),i={name:"training/linux/disk-management.md"};function l(t,s,c,d,o,r){return p(),n("div",{"data-pagefind-body":!0},s[0]||(s[0]=[e(`<h1 id="linux-磁盘管理-记一次分区格式化" tabindex="-1">Linux 磁盘管理 - 记一次分区格式化 <a class="header-anchor" href="#linux-磁盘管理-记一次分区格式化" aria-label="Permalink to &quot;Linux 磁盘管理 - 记一次分区格式化&quot;">​</a></h1><blockquote><p>注意：</p><ul><li>数据无价，谨慎操作！！！</li><li>具体业务敏感部分已做脱敏处理，示例服务器域为 <code>mufeng.dev</code></li></ul></blockquote><p>我们有个业务项目使用华为 <strong>FusionCompute</strong> 搭建了一套私有云平台，属于比较高的配置，该教程记录一次操作系统加磁盘后进行分区格式化的操作。</p><ul><li>虚拟化平台：华为 Fusion Compute</li><li>操作系统版本：AlmaLinux 9.5</li></ul><h2 id="一、准备工作" tabindex="-1">一、准备工作 <a class="header-anchor" href="#一、准备工作" aria-label="Permalink to &quot;一、准备工作&quot;">​</a></h2><h3 id="_1-1-简介" tabindex="-1">1.1 简介 <a class="header-anchor" href="#_1-1-简介" aria-label="Permalink to &quot;1.1 简介&quot;">​</a></h3><p>平台图形化界面新增磁盘，此处不再赘述，新增后进入系统，查看属性如下：</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>[root@mufeng /]# df -h</span></span>
...
<span class="line"><span>/dev/mapper/almalinux-swap none                    swap    defaults        0 0</span></span></code></pre></div><p>该文件格式为：<code>&lt;设备（或者其UUID）&gt; &lt;挂载点&gt; &lt;文件系统&gt; &lt;挂载选项&gt; &lt;备份频率&gt; &lt;检查顺序&gt;</code>，详见官方文档。</p><p>挂载光驱的写法示例：<code>/dev/cdrom /mnt/cdrom iso9660 defaults 0 0</code></p><p>挂载文件后，我们可以使用 <code>blkid</code> 命令查看磁盘对应 id。</p><p>编辑该文件后，执行 <code>mount -a</code> 即可将该文件规定的挂载点重新挂载。</p><h2 id="二、开始操作" tabindex="-1">二、开始操作 <a class="header-anchor" href="#二、开始操作" aria-label="Permalink to &quot;二、开始操作&quot;">​</a></h2><ol><li><p>使用 parted 进行分区操作。</p><div class="language-text vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">text</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>[root@mufeng ~]# parted /dev/vdb</span></span>
<span class="line"><span>GNU Parted 3.5</span></span>
<span class="line"><span>Using /dev/vdb</span></span>
...
<span class="line"><span>Information: You may need to update /etc/fstab.</span></span></code></pre></div></li><li><p>开始分区</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>[root@mufeng mufeng]# mkfs.xfs /dev/vdb1 </span></span>
...
<span class="line"><span>UUID=71d3726a-61b8-4d2f-8be3-a3e0fe9f63da       /mufeng/data         xfs     defaults        0 0</span></span></code></pre></div></li><li><p>执行挂载</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>[root@mufeng mufeng]# mount -a</span></span></code></pre></div></li><li><p>查看结果</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>[root@mufeng mufeng]# lsblk</span></span>
<span class="line"><span>NAME               MAJ:MIN RM  SIZE RO TYPE MOUNTPOINTS</span></span>
<span class="line"><span>sr0                 11:0    1    2G  0 rom  </span></span>
<span class="line"><span>vda                252:0    0  100G  0 disk </span></span>
<span class="line"><span>├─vda1             252:1    0    1G  0 part /boot</span></span>
...
<span class="line"><span>└─vdb2             252:18   0  700G  0 part /mufeng/data</span></span></code></pre></div></li></ol><p>至此，该磁盘的新建以及分区挂载操作完成！</p>`,57)]))}const b=a(i,[["render",l]]);export{u as __pageData,b as default};
